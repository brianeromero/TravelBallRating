...
final class AppDayOfWeekViewModel: ObservableObject {
    ...
    func updateDayAndFetch(day: DayOfWeek) async {
        guard let team = selectedTeam else {
            print("Team is not set.")
            return
        }
        
        // Assuming fetchCurrentDayOfWeek returns values and might do async work
        let (appDayOfWeek, matTimes) = await fetchCurrentDayOfWeek(for: team, day: day, selectedDayBinding: Binding(get: { self.selectedDay }, set: { self.selectedDay = $0 ?? .monday }))
    ...
    func saveAppDayOfWeekLocally() {
        guard let team = selectedTeam,
            ...
            errorMessage = "team, AppDayOfWeek, or DayOfWeek is not selected."
            print("team, AppDayOfWeek, or DayOfWeek is not selected.")
            return
        }
        
        // Check if name is nil and generate it if necessary
        if appDayOfWeek.name == nil {
            appDayOfWeek.name = AppDayOfWeekRepository.shared.generateName(for: team, day: dayOfWeek)
        }
        
        // Use the repository to save the AppDayOfWeek data
        repository.updateAppDayOfWeek(appDayOfWeek, with: team, dayOfWeek: dayOfWeek, context: viewContext)
    }
            ...
            PersistenceController.shared.container.performBackgroundTask { backgroundContext in
                backgroundContext.perform {
                    do {
                        ...
                        // Use the AppDayOfWeek extension to get full Firestore data, including matTimes and team
    ...
    func saveAppDayOfWeek() async { // Make it async
        guard let selectedTeam = selectedTeam,
            ...
            print("🚫 Missing data for saving AppDayOfWeek: team: \(selectedTeam != nil), DayOfWeek: \(selectedDay != nil), AppDayOfWeek: \(currentAppDayOfWeek != nil)")
            return
        }
        ...
        do {
            ...
            try await saveAppDayOfWeekToFirestore(
                selectedTeamID: selectedTeam.objectID,
    ...
    func ensureInitialization() async {
        if selectedTeam == nil {
            errorMessage = "Team is not selected."
            print("Error: Team is not selected.")
            return
        }
        ...
        let (appDayOfWeek, matTimes) = await fetchCurrentDayOfWeek(for: selectedTeam!, day: selectedDay, selectedDayBinding: Binding(get: { self.selectedDay }, set: { self.selectedDay = $0 ?? .monday }))
    ...
    @MainActor
    func fetchCurrentDayOfWeek(for team: Team, day: DayOfWeek, selectedDayBinding: Binding<DayOfWeek?>) async -> (AppDayOfWeek?, [MatTime]?) {
        print("Fetching current day of week for team: \(team.teamName ?? ""), day: \(day)")
        
        let context = repository.getViewContext()
        
        // Step 1: Check Core Data
        if let existingAppDayOfWeek = repository.fetchAppDayOfWeek(for: day.rawValue, team: team, context: context) {
        ...
        do {
            ...
            if let data = document.data() {
                ...
                if let newAppDayOfWeek = repository.fetchOrCreateAppDayOfWeek(for: day.rawValue, team: team, context: context) {
        ...
        alertMessage = "No mat times have been entered for \(day.displayName) at \(team.teamName ?? "this team")."
        showAlert = true
    ...
    func addOrUpdateMatTime(
        ...
        guard selectedTeam != nil else {
            print("Error: Selected team is not set. Please select a team before adding a mat time.")
            return
        }
    ...
    func refreshMatTimes() async {
        ...
        if let selectedTeam = selectedTeam, let unwrappedSelectedDay = selectedDay {
            // Fetch and assign the current day of the week
            let (appDayOfWeek, matTimes) = await fetchCurrentDayOfWeek(for: selectedTeam, day: unwrappedSelectedDay, selectedDayBinding: Binding(get: { self.selectedDay }, set: { self.selectedDay = $0 ?? .monday }))
            ...
            print("Error: Either team or day is not selected.")
        }
    ...
    func updateMatTime(_ matTime: MatTime) async throws {
        ...
        guard let selectedTeam = self.selectedTeam else {
            throw NSError(
                ...
                code: 5,
                userInfo: [
                    NSLocalizedDescriptionKey: "Missing selectedTeam for Firestore sync."
                ]
            )
        }
    ...
    // MARK: - Update Day
    func updateDay(for team: Team, dayOfWeek: DayOfWeek) async {
        print("Updating day settings for team: \(team) and dayOfWeek: \(dayOfWeek)")
        
        // Fetch or create the AppDayOfWeek instance with context
        let appDayOfWeek = repository.fetchOrCreateAppDayOfWeek(for: dayOfWeek.rawValue, team: team, context: viewContext)
    ...
    @MainActor
    func loadSchedules(for team: Team) async -> Bool {
        guard let day = selectedDay else { return false }
        print("LOAD_SCHEDULES: START for team: \(team.teamName ?? "Unknown") and day: \(day.displayName)")

        guard let appDayOfWeeks = team.appDayOfWeeks as? Set<AppDayOfWeek> else {
            matTimesForDay[day] = []
            schedules[day] = []
            return false
        }
        ...
        print("Loaded \(matTimes.count) mat times for \(day.displayName) at \(team.teamName ?? "Unknown")")
        ...
        let islandObjectIDsTempDict = await withTaskGroup(of: (DayOfWeek, [NSManagedObjectID]).self) { group -> [DayOfWeek: [NSManagedObjectID]] in
            ...
            for day in DayOfWeek.allCases {
                group.addTask { [self] in // Capture self strongly for the async task
                    print("TaskGroup: Fetching team ObjectIDs for day: \(day.rawValue)")
                    ...
                    do {
                        ...
                        print("TaskGroup: Error fetching team ObjectIDs for day \(day.rawValue): \(error.localizedDescription)")
            ...
            print("TaskGroup: Completed fetching all team ObjectIDs. Total days processed: \(result.count)")
            return result
        }
            ...
            for (day, objectIDs) in islandObjectIDsTempDict {
                ...
                for objectID in objectIDs {
                    do {
                        ...
                        guard let team = try self.viewContext.existingObject(with: objectID) as? Team else {
                            print("    ❌ MainActor: Failed to rehydrate Team with ID: \(objectID)")
                            continue
                        }

                        // Now that 'team' is rehydrated on the MainActor, safely access its relationships and properties
                        guard let appDayOfWeeks = team.appDayOfWeeks as? Set<AppDayOfWeek> else {
                            print("    ⚠️ MainActor: Team \(team.teamName ?? "Unnamed") has no AppDayOfWeeks relationship.")
                            continue
                        }
    ...
    func fetchAppDayOfWeekAndUpdateList(for team: Team, day: DayOfWeek, context: NSManagedObjectContext) {
        print("Fetching AppDayOfWeek for team: \(team.teamName ?? "Unknown") and day: \(day.displayName)")
        
        // Fetching the AppDayOfWeek entity
        if let appDayOfWeek = repository.fetchAppDayOfWeek(for: day.rawValue, team: team, context: context) {
            ...
            print("No AppDayOfWeek found for \(day.displayName) on team \(team.teamName ?? "Unknown")")
        }
    ...
    func addNewMatTime() async {
        guard let day = selectedDay, let team = selectedTeam else {
            errorMessage = "Day of the week or team is not selected."
            print("Error: Day of the week or team is not selected.")
            return
        }
        ...
        appDayOfWeek.name = "\(team.teamName ?? "Unknown team") \(day.displayName)"
    ...
    func updateBindings() {
        ...
        print("Selected team: \(selectedTeam?.teamName ?? "None")")
    ...
    func setSelectedTeam(_ team: Team) {
        ...
        print("Selected team set to: \(team)")
    }
    ...
    func addMatTimes(day: DayOfWeek,
        ...
        guard let appDay = repository.fetchOrCreateAppDayOfWeek(for: day.rawValue, team: team, context: viewContext) else {
    ...
    func addMatTime(matTime: MatTime? = nil, for day: DayOfWeek) async {
        guard let team = selectedTeam else {
            errorMessage = "Selected team is not set."
            print("Error2: Selected team is not set.")
            return
        }
        ...
        let appDayOfWeek = repository.fetchOrCreateAppDayOfWeek(for: day.rawValue, team: team, context: viewContext)
    ...
    func updateSchedules() async {
        guard let selectedTeam = self.selectedTeam else { return }
        guard let selectedDay = self.selectedDay else { return }

        print("Updating schedules for team: \(selectedTeam) and day: \(selectedDay)")
                ...
                PersistenceController.shared.container.performBackgroundTask { backgroundContext in
                    do {
                        ...
                        for document in querySnapshot.documents {
                            let teamID = document.data()["id"] as? String ?? document.documentID
                            ...
                            if let team = existingIsland {
                                ...
                                newTeam.configure(document.data())
                                newTeam.teamID = UUID(uuidString: teamID) ?? UUID()
                                firestoreUpdateList.append((newTeam, document.reference))
                                print("    ➡️ Created new team: \(newTeam.teamName ?? "Unnamed")")
                            }
                        }
    ...
    @MainActor
    func updateCurrentDayAndMatTimes(for team: Team, day: DayOfWeek) async {
        do {
            let document = try await firestore.collection("appDayOfWeek")
...
extension Team {
    func configure(_ data: [String: Any]) {
        // Map Firestore document data to Team properties
        if let teamIDString = data["teamID"] as? String {
            self.teamID = UUID(uuidString: teamIDString) // Convert String to UUID
        }
        self.teamName = data["teamName"] as? String
        self.teamLocation = data["teamLocation"] as? String // Corrected to teamLocation
...
extension AppDayOfWeek {
    func toFirestoreData() -> [String: Any] {
        var data: [String: Any] = [
            ...
            "team": self.team?.toFirestoreData() ?? [:], // Ensure to include Team data
        ]
