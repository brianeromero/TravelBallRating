...
class FirestoreSyncManager {
    ...
    func syncInitialFirestoreData() async {
        do {
            ...
            // 1️⃣ Teams (parent of AppDayOfWeek)
            try await downloadCollection(db: db, name: "teams")
            ...
            // 4️⃣ Reviews (depends on Team only)
    ...
    private func uploadLocalRecordsToFirestore(collectionName: String, records: [String]) async {
        ...
        let (uploadedCount, errorCount) = await Task.detached(priority: .background) { () -> (Int, Int) in
            ...
            for record in records {
                ...
                case "teams":
                    guard let team = localRecord as? Team else { continue }
                    recordData = [
                        "id": team.teamID?.uuidString ?? "",
                        "name": team.teamName,
                        "location": team.teamLocation,
                        "country": team.country ?? "",
                        "createdByUserId": team.createdByUserId ?? "",
                        "createdTimestamp": team.createdTimestamp ?? Date(),
                        "teamWebsite": team.teamWebsite?.absoluteString ?? "",
                        "latitude": team.latitude,
                        "longitude": team.longitude,
                        "lastModifiedByUserId": team.lastModifiedByUserId ?? "",
                        "lastModifiedTimestamp": team.lastModifiedTimestamp ?? Date()
                    ]
                case "reviews":
                    ...
                    recordData = [
                        ...
                        "createdTimestamp": review.createdTimestamp as Any,
                        "teamID": review.team?.teamID?.uuidString ?? ""
                    ]
                case "MatTime":
    ...
    private func downloadFirestoreRecordsToLocal(collectionName: String, records: [String]) async {
        ...
        for record in records {
            ...
            do {
                ...
                switch collectionName {
                case "teams":
                    Self.syncTeamStatic(docSnapshot: docSnapshot, context: context)
                case "reviews":
    ...
    // Team
    // ---------------------------
    private static func syncTeamStatic(
        ...
        let teamName = data["teamName"] as? String ?? data["name"] as? String
        let teamLocation = data["teamLocation"] as? String ?? data["location"] as? String
        
        guard let name = teamName, let location = teamLocation else {
            Task { @MainActor in
                FirestoreSyncManager.log(
                    "⚠️ Missing required fields for Team \(docSnapshot.documentID). Skipping.",
                    level: .error,
                    collection: "teams"
                )
            }
        ...
        let teamWebsiteString = data["teamWebsite"] as? String
        let teamWebsite = teamWebsiteString != nil ? URL(string: teamWebsiteString!) : nil
        
        context.perform {
            let fetchRequest = Team.fetchRequest()
            if let uuid = UUID(uuidString: docSnapshot.documentID) {
                fetchRequest.predicate = NSPredicate(format: "teamID == %@", uuid as CVarArg)
            } else {
                fetchRequest.predicate = NSPredicate(format: "teamIDString == %@", docSnapshot.documentID)
            }
            
            do {
                let results = try context.fetch(fetchRequest)
                let team = results.first ?? Team(context: context)
                
                team.teamID = UUID(uuidString: docSnapshot.documentID)
                team.teamName = name
                team.teamLocation = location
                team.country = country
                team.createdByUserId = createdByUserId
                team.createdTimestamp = createdTimestamp
                team.lastModifiedByUserId = lastModifiedByUserId
                team.lastModifiedTimestamp = lastModifiedTimestamp
                team.latitude = latitude
                team.longitude = longitude
                team.teamWebsite = teamWebsite
                
                if context.hasChanges {
                    try context.save()
                    Task { @MainActor in
                        FirestoreSyncManager.log(
                            "✅ Synced teams record: \(docSnapshot.documentID)",
                            level: .success,
                            collection: "teams"
                        )
                    }
                    ...
                    Task { @MainActor in
                        FirestoreSyncManager.log(
                            "ℹ️ No changes detected for Team \(docSnapshot.documentID), skipping save.",
                            level: .info,
                            collection: "teams"
                        )
                    }
                ...
                Task { @MainActor in
                    FirestoreSyncManager.log(
                        "❌ Failed syncing team \(docSnapshot.documentID): \(error)",
                        level: .error,
                        collection: "teams"
                    )
                }
    ...
    private static func syncReviewStatic(
        ...
        context.perform {
            ...
            if let teamIDString = data["teamID"] as? String {
                let teamUUID = UUID(uuidString: teamIDString) ?? UUID.fromStringID(teamIDString)
                let teamFetch: NSFetchRequest<Team> = Team.fetchRequest()
                teamFetch.predicate = NSPredicate(format: "teamID == %@", teamUUID as CVarArg)
                teamFetch.fetchLimit = 1
                ...
                if let team = try? context.fetch(teamFetch).first {
                    ...
                    Task { @MainActor in
                        FirestoreSyncManager.log(
                            "⚠️ Team not found for review \(documentID) (teamID: \(teamIDString))",
                            level: .warning,
                            collection: "Review"
                        )
                    }
        ...
        context.perform {
            ...
            do {
                ...
                if let nameFromFS = docSnapshot.get("name") as? String {
                    ado.name = nameFromFS
                } else if let teamName = (docSnapshot.get("team") as? [String: Any])?["teamName"] as? String {
                    ado.name = "\(teamName) - \(day)"
                } else {
                    ado.name = day
                }
                ...
                // --- Link Team
                if let teamData = docSnapshot.get("team") as? [String: Any],
                   let teamIDString = teamData["teamID"] as? String {
                    
                    let teamUUID = UUID.fromStringID(teamIDString)
                    ...
                    let team: Team
                    if let existingTeam = try context.fetch(teamFetch).first {
                        team = existingTeam
                    } else {
                        team = Team(context: context)
                        team.teamID = teamUUID
                        team.teamName = (teamData["teamName"] as? String)
                                        ?? (teamData["name"] as? String)
                                        ?? "Unknown Team"
                        team.teamLocation = (teamData["teamLocation"] as? String)
                                            ?? "Unknown Location"
                        team.country = teamData["country"] as? String
                        team.createdTimestamp = (teamData["createdTimestamp"] as? Timestamp)?.dateValue() ?? Date()
                        team.latitude = teamData["latitude"] as? Double ?? 0.0
                        team.longitude = teamData["longitude"] as? Double ?? 0.0
                        if let urlString = teamData["teamWebsite"] as? String {
                            team.teamWebsite = URL(string: urlString)
                        }
                    }
...
extension FirestoreSyncManager {
    ...
    func startFirestoreListeners() {
        ...
        listenToCollection("teams", handler: Self.handleTeamChange)
...
extension FirestoreSyncManager {
    ...
    static func handleTeamChange(_ change: DocumentChange, _ context: NSManagedObjectContext) {
        switch change.type {
        case .added, .modified:
            syncTeamStatic(docSnapshot: change.document, context: context)
        case .removed:
            deleteEntity(ofType: Team.self, idString: change.document.documentID, keyPath: \.teamID, context: context)
        }
    }
