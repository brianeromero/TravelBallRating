...
struct RatingSection: View {
    ...
    var body: some View {
        Section(header: Text("Rate the team")) {
            HStack {
...
// Main view for team Mat Review
struct TeamPracticeReviewView: View {
    @Binding var selectedTeamID: UUID?
    @State private var isReviewsFetched = false
    @State private var showReview = false // This might be redundant if TeamPracticeReviewView's presentation is managed by NavigationLink
    ...
    @State private var cachedTeam: Team?
    ...
    init(selectedTeamID: Binding<UUID?>, callerFile: String = #file, callerFunction: String = #function) {
        self._selectedTeamID = selectedTeamID

        os_log(
            "TeamPracticeReviewView INITIALIZED from %{public}s, function: %{public}s",
            log: logger,
            type: .info,
    ...
    var body: some View {
        ...
        .navigationTitle("Add Team Review")
        .onAppear {
            os_log("TeamPracticeReviewView ON APPEAR for team: %@", log: logger, type: .info, selectedTeamInternal?.teamName ?? "nil")
            print("🟢 TeamPracticeReviewView ON APPEAR for team: \(selectedTeamInternal?.teamName ?? "nil")")
            

        }
        .onDisappear {
            os_log("TeamPracticeReviewView ON DISAPPEAR - selectedTeamInternal: %@", log: logger, type: .info, selectedTeamInternal?.teamName ?? "nil")
            print("🔴 TeamPracticeReviewView ON DISAPPEAR - selectedTeamInternal: \(selectedTeamInternal?.teamName ?? "nil")")

        }
        .task(id: selectedTeamInternal?.objectID) {
            print("📘 TeamPracticeReviewView .task started. Team: \(selectedTeamInternal?.teamName ?? "nil")")

            
            guard let team = selectedTeamInternal else {
                os_log("TeamPracticeReviewView .task: selectedTeamInternal is nil, returning.", log: logger, type: .info)
                return
            }
            guard cachedTeam?.objectID != team.objectID else {
                os_log("TeamPracticeReviewView .task: Team selection unchanged — skipping task for %@", log: logger, type: .info, team.teamName ?? "nil")
                return
            }

            os_log("TeamPracticeReviewView .task: Team selection changed to: %@", log: logger, type: .info, team.teamName ?? "Unknown team")

            cachedTeam = team
            isReviewsFetched = false // Reset for new team
            //onTeamChange(team)
            ...
            os_log("TeamPracticeReviewView .task: Average rating for team %@: %.2f", log: logger, type: .info, team.teamName ?? "Unknown", cachedAverageRating)

            os_log("TeamPracticeReviewView .task: Team selection change completed", log: logger, type: .info)
        }
    }
    ...
    private func submitReview() {
        ...
        Task {
            if let currentUser = await authViewModel.getCurrentUser() {
                ...
                self.isLoading = true
                os_log("Submitting review for team: %@", log: logger, type: .info, team.teamName ?? "Unknown")
                ...
                do {
                    ...
                    self.cachedAverageRating = Double(await ReviewUtils.fetchAverageRating(for: team, in: self.viewContext, callerFunction: #function))
