...
struct CustomMapMarkerPreview: View {
    ...
    @State private var cameraPosition: MapCameraPosition = {
        let coordinate = CLLocationCoordinate2D(latitude: MockTeam.example.latitude, longitude: MockTeam.example.longitude)
    ...
    var body: some View {
        ZStack {
            Map(position: $cameraPosition) {
                ...
                Annotation(previewTeam.teamName ?? "Preview Team", coordinate: CLLocationCoordinate2D(latitude: previewTeam.latitude, longitude: previewTeam.longitude), anchor: .center) {
                    // This VStack combines the team name text and your CustomMarkerView
                    VStack {
                        Text(previewTeam.teamName ?? "Unnamed Team")
                            .font(.caption)
                            .padding(5)
                    ...
                    .onTapGesture {
                        print("Tapped on preview team: \(previewTeam.teamName ?? "Unnamed")")
                    }
                }
            }
            .mapControls {
...
// You'll need to ensure your 'Team' type is defined elsewhere in your project.
// This 'MockTeam' struct serves as a stand-in for preview purposes.
struct MockTeam: Identifiable {
    let id = UUID()
    var teamName: String? = "Your Mom's team"
    var latitude: Double = 34.0522 // Example: Los Angeles latitude
    var longitude: Double = -118.2437 // Example: Los Angeles longitude

    static let example = MockTeam()
}
