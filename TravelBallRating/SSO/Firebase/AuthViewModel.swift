...
class AuthViewModel: ObservableObject {
    ...
    private let logger = os.Logger(subsystem: "com.TravelBallRating.app", category: "AuthViewModel")
...
///PART2
    ...
    func mapFirebaseUserToTravelBallRatingUser(firebaseUser: FirebaseAuth.User, userName: String, name: String) async throws -> User {
        ...
        let travelBallRatingUser = UserInfo(entity: entityDescription, insertInto: context)
        travelBallRatingUser.email = firebaseUser.email ?? ""
        travelBallRatingUser.userName = userName
        travelBallRatingUser.name = name
        travelBallRatingUser.userID = firebaseUser.uid
        
        // Hash the password
        let hashedPassword = try hashPassword.hashPasswordScrypt(formState.password)
        travelBallRatingUser.passwordHash = hashedPassword.hash
        travelBallRatingUser.salt = hashedPassword.salt
        travelBallRatingUser.iterations = Int64(hashedPassword.iterations)
        travelBallRatingUser.isVerified = false
        
        // Save the Core Data context to persist the new UserInfo
        try context.save()
        
        // Convert the saved UserInfo to your User struct
        return User.fromUserInfo(travelBallRatingUser)
    }
